...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col_left:
        ...
        수기전표_files = st.file_uploader(
            "수기전표 PDF (월별 여러 개 한번에 선택 가능) — 2단계 검증에서 사용",
            type=["pdf"], accept_multiple_files=True, key="step1_수기"
        )
    ...
    if run_btn and 매입매출장_file:
        ...
        try:
            ...
            for _, data in st.session_state.shared_환급_files:
                pass  # read 했으면 seek
            st.session_state.shared_수기전표_files = [
                (uf.name, uf.read()) for uf in (수기전표_files or [])
            ]
